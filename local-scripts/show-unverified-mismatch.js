...
async function showUnverifiedMismatchStudents() {
    console.log("🔍 กำลังค้นหาคนที่รหัสนักศึกษาตรง แต่ชื่อไม่ตรง และ 'ยังไม่ได้ Verify' เท่านั้น...\n");
        ...
        if (matchingFbUsersById.length > 0) {
            ...
            if (!foundExactMatch) {
                for (const fbUser of matchingFbUsersById) {
                    ...
                    // เงื่อนไขใหม่: ถ้า verify ไปแล้ว ให้ข้ามเลย ไม่ต้องเอามาโชว์
    ...
    console.log(`\n====== 📝 สรุป ======`);
    if (mismatchCount === 0) {
        console.log(`🎉 เยี่ยมมาก! ไม่มีรายชื่อไหนที่ตกค้างหรือรอ Verify แล้วครับ`);
    } else {
        console.log(`พบคนที่ชื่อพิมพ์คลาดเคลื่อน และยังไม่ได้ Verify ทั้งหมด: ${mismatchCount} รายการ`);
    }
    console.log(`=====================\n`);
}

showUnverifiedMismatchStudents();
