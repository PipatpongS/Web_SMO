...
# ==============================================================================
# 📊 ตรวจสอบ Quota ข้อความของ LINE Official Account
# ==============================================================================
def get_remaining_quota():
    try:
        req_q = urllib.request.Request('https://api.line.me/v2/bot/message/quota', headers={'Authorization': f'Bearer {LINE_ACCESS_TOKEN}'})
        req_c = urllib.request.Request('https://api.line.me/v2/bot/message/quota/consumption', headers={'Authorization': f'Bearer {LINE_ACCESS_TOKEN}'})
        
        with urllib.request.urlopen(req_q) as r1:
            q_data = json.loads(r1.read().decode('utf-8'))
        with urllib.request.urlopen(req_c) as r2:
            c_data = json.loads(r2.read().decode('utf-8'))
            
        total_limit = q_data.get('value', 300)
        total_used = c_data.get('totalUsage', 0)
        remaining = total_limit - total_used
        return max(0, remaining), total_limit, total_used
    except Exception as e:
        print(f"⚠️ ไม่สามารถดึงข้อมูล Quota ได้: {e}")
        return 300, 300, 0
...
def send_multicast_message(target_uids, flex_msg):
    ...
    try:
        ...
        return {'success': False, 'error': f"HTTP {e.code}: {error_body}", 'count': len(target_uids)}
    except Exception as e:
...
def main():
    ...
    remaining_quota, total_limit, total_used = get_remaining_quota()
    print(f"📊 โควต้าข้อความ LINE เดือนนี้: ใช้ไปแล้ว {total_used}/{total_limit} ข้อความ (คงเหลือ {remaining_quota} ข้อความ)")
    ...
    if DRY_RUN:
        ...
        if len(target_uids) > remaining_quota:
            print(f"⚠️ รายชื่อทั้งหมด ({len(target_uids)} คน) เกินโควต้าคงเหลือ ({remaining_quota} ข้อความ)")
            print(f"✂️ ระบบจะจำกัดการส่งให้พอดีกับโควต้าคงเหลือสูงสุด {remaining_quota} คน เพื่อป้องกัน HTTP 429 Error")
            target_uids = target_uids[:remaining_quota]

    if not target_uids:
        print("⚠️ ไม่พบรายชื่อ LINE UID ในการส่งข้อความ หรือโควต้าหมดแล้ว")
        return
